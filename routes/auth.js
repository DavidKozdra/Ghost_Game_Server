...
AuthLoginRoute.post("/login", async function (req, res) {
    let db_connect = dbo.getDbGhost_game
...
AuthLoginRoute.get("/session", async function (req, res) {
    res.json({ isLoggedIn: !!req.session.user, ...req.session.user })
})

AuthLoginRoute.post("/logout", async function (req, res) {
